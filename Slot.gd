...
func _process(delta):
	if inventory_Base_node.get_meta("ItemHeld") && sprite_following_mouse:
		...
		if inventory_Base_node.get_meta("ItemHeld"):
			if inventory[index] == null:
				if sprite_following_mouse:
					var item_info = sprite_following_mouse.get_meta("HeldItemInfo")
					print(inventory[index])
					inventory[index] = item_info
					Slot_update()
...
func follow_mouse_sprite():
	...
	sprite_following_mouse.set_meta("HeldItemInfo", inventory[index])
